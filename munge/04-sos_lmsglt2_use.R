...
lmiu <- function(atc, treatname, lmdata = lmsglt2) {
  treatvar <- paste0("sos_ddr_", treatname)
  treatiu <- paste0("sos_ddr_", treatname, "_iu")
  ...
  lmdata %>%
    ...
    filter(
      ...
      EDATUM > global_indexdtm - 30.5 * 5 - 365,
    ),
  by = "LopNr"
  )
  ...
  lmtmp <- lmtmp %>%
    ...
    slice(1) %>%
    ungroup() %>%
    mutate(tmp_prevuser = TRUE) %>%
    select(LopNr, tmp_prevuser)
  ...
  pop <<- left_join(pop,
    ...
    mutate(!!treatiu := if_else(!is.na(tmp_prevuser), NA_character_, !!sym(treatvar))) %>%
    select(-tmp_prevuser)
}

lmiu("^(A10BK|A10BD1[5-6]|A10BD19|A10BD2[0-1]|A10BD2[3-5]|A10BX09|A10BX11|A10BX12)", "sglt2i")

lmiu("^(A10BJ|A10BX04|A10BX07|A10BX10|A10BX1[3-4])", "glp1a")

lmiu("^(A10BH|A10BD0[7-9]|A10BD1[0-3]|A10BD1[8-9]|A10BD2[1-2]|A10BD2[4-5])", "dpp4i")
